    ...
    public ApiRespDto<?> sendMail(SendMailReqDto sendMailReqDto, PrincipalUser principalUser) {
        ...
        public Map<String, Object> verify(String token) {
            ...
            try{
                ...
                Optional<UserRole> optionalUserRole = userRoleRepository.getUserRoleByUserIdAndRoleId(user.getUserId(), 3);

                if(optionalUserRole.isEmpty()) {
                    resultMap = Map.of("status", "failed", "message", "이미 인증이 완료된 메일입니다.");
                } else {
                    userRoleRepository.updateRoleId(optionalUserRole.get().getRoleId(), userId);
